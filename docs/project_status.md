...
| Current phase | Leaky BR-CUSUM drift-control closing evaluation |
| Current branch | `phase/05b-leaky-br-cusum` |
| Last PR | pending Draft — Leaky BR-CUSUM follow-up to BR-CUSUM |
| Last review | Nine-dataset fixed-window Leaky BR-CUSUM evaluation validated |
...
- [x] Nine-dataset Leaky BR-CUSUM (`rho=0.95`, `k=0.50`) drift-control comparison
...
- Leaky BR-CUSUM did not eliminate long non-zero drift and its lower channel remained inactive under the fixed main protocol. This closes new detector-method experiments.

## Next Phase

Move to final report and material preparation. Use baseline, EWMA, BR-CUSUM and Leaky BR-CUSUM summaries; discuss the fixed calibration split, extremes, persistent drift and inactive lower channel. Do not introduce a fourth detector method.
